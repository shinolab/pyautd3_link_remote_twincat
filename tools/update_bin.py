...
def download_and_extract(repo: str, name: str, version: str) -> None:
    ...
    dst_dir = Path("pyautd3_link_remote_twincat/bin")
...
def should_update_dll(version: str) -> bool:
    if platform.system() == "Windows":
        if not Path("pyautd3_link_remote_twincat/bin/autd3capi_link_remote_twincat.dll").exists():
            return True
    elif platform.system() == "Darwin":
        if not Path("pyautd3_link_remote_twincat/bin/libautd3capi_link_remote_twincat.dylib").exists():
            return True
    elif not Path("pyautd3_link_remote_twincat/bin/libautd3capi_link_remote_twincat.so").exists():
        return True
...
def copy_dll() -> None:
    with Path("pyautd3_link_remote_twincat/__init__.py").open() as f:
        content = f.read()
    ...
    download_and_extract("autd3-capi-link-remote-twincat", "autd3-capi-link-remote-twincat", version)
    shutil.copyfile("LICENSE", "pyautd3_link_remote_twincat/LICENSE.txt")
    shutil.copyfile("ThirdPartyNotices.txt", "pyautd3_link_remote_twincat/ThirdPartyNotices.txt")
